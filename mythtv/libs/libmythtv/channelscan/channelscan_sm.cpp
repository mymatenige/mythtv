    ...
    if (transport_tune_complete)
    {
        ...
        if (!m_currentInfo->IsEmpty())
        {
            ...
            LOG(VB_CHANSCAN, LOG_DEBUG, LOC +
                ...
                QString("tunerType:%1 %2 ").arg(static_cast<int>(m_scanDTVTunerType)).arg(m_scanDTVTunerType.toString()) +
                QString("m_modSys:%1 %2 ").arg(static_cast<int>(item.m_tuning.m_modSys)).arg(item.m_tuning.m_modSys.toString()) +
...
bool ChannelScanSM::AddToList(uint mplexid)
{
    ...
    LOG(VB_CHANSCAN, LOG_DEBUG, LOC +
        QString("tunertype:%1 %2 sourceid:%3 sistandard:%4 fn:'%5' mplexid:%6")
            .arg(static_cast<int>(tt)).arg(tt.toString()).arg(sourceid).arg(sistandard, fn).arg(mplexid));
