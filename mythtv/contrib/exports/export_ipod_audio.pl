...
my $export_file = $export_dir."/".$new_name."_".$new_title."_".$new_subtitle."_".$new_starttime.".mp3";
...
# MP3, 160Kbps, stereo, 48kHz
$command = "/bin/nice -n19 /usr/bin/ffmpeg -i '$import_file' -strict experimental -f mp3 -acodec libmp3lame -ab '$max_bitrate' -ac 2 -ar 48k -y '$export_file' > /dev/null 2>&1";
print "$command\n";
system "$command";

my $year = substr($new_starttime, 0, 4);
$command = "/usr/bin/id3tag --song='$title $starttime' --album='$subtitle' --artist='$name' --year='$year' '$export_file'";
print "$command\n";
system "$command";
